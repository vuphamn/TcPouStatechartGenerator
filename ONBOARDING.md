...
| Editors | `MethodStructuredTextEditor.tsx`, `PouCodeEditor.tsx` (the POU's own declaration and body, `utils/pouBody.ts`), `DutEnumEditor.tsx`, `StructuredTextCodeEditor.tsx` (the shared code view: caret line, zoom from `hooks/useEditorZoom.ts`), `utils/st*.ts` |
